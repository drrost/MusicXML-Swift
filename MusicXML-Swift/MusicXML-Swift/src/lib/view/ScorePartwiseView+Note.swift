...
extension ScorePartwiseView {
    ...
    func draw(note: Note, context: CGContext, point: CGPoint, scale: CGFloat = 1) {
        ...
        draw(noteWith: note.type, context: context)

        context.restoreGState()
    }

    fileprivate func draw(noteWith type: NoteType, context: CGContext) {
        switch type {
        case .whole:
            drawWhole()
        case .quarter:
    ...
    fileprivate func drawQuater(context: CGContext) {
        ...
        let noteHeadPath = UIBezierPath(ovalIn: CGRect(x: -1.0, y: 3.0, width: 15.3, height: 10.5))
